...
export const get_comments_by_user = async (req, res) => {
  try {
    const comments = await Comment.find(
      { user: req.query.user },
      "user edit content post date"
    );
    if (comments) {
      res.status(200).json(comments);
    } else {
      res.status(400).json({
        message: `Cant find comments with user ${req.query.user}`,
      });
    }
  } catch (err) {
    res.status(500).json({
...
export const update_comment_by_id = async (req, res) => {
  try {
    Comment.findByIdAndUpdate(
      req.params.id,
      { content: req.body.content, date: Date.now(), edit: true },
      { new: true },
      (err, result) => {
        if (err) {
          res.status(400).json({
      ...
      Post.findByIdAndUpdate(
        ...
        (err, result) => {
          if (err) {
            ...
            res
              .status(200)
              .json({ message: `Comment ${comment._id} deleted!` });
          }
        }
      );
    } else {
      res.status(400).json({
        message: `Not deleting comment ${req.params.id} since id does not exist!`,
      });
    }
  } catch (err) {
    res.status(500).json({
